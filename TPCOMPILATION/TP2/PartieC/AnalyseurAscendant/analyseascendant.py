...
#This function takes in the grammar rules, non-terminal symbols, and the start symbol as input.
#It augments the grammar by adding a '.' symbol to each production rule to indicate the current parsing position.
#The augmented grammar is returned as a list of rules.
...
# The closure is calculated by iteratively expanding the set of rules that can be derived from the current state.
...
#It finds all symbols on which GOTO needs to be performed and makes recursive calls to the GOTO function.
...
#This function performs the GOTO operation for a given state and the next symbol following the dot '.'.
...
#It populates the parsing table with shift and reduce actions based on the computed states, GOTO transitions, FIRST, and FOLLOW sets.
def createParseTable(statesDict, stateMap, T, NT):
	...
	for y in Table:
		...
		print(f"{{:>3}} {frmt1.format(*y)}"
			.format('I'+str(j)))
		j += 1
#prints the given list of rules i
def printResult(rules):
	for rule in rules:
		print(f"{rule[0]} ->"
			f" {' '.join(rule[1])}")
#prints all the GOTO transitions computed
